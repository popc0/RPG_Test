...
public static class SkillAssetTool
{
    ...
    public static int GetUniqueRandomSerial()
    {
        ...
        do
        {
            newSerial = Random.Range(1, 999999);
            maxTry--;
        }
    ...
    public static void CreateEvolutionAsset(SkillData baseData)
    {
        ...
        // ★ 防呆：如果父技能還是 0 (新技能)，先幫它生一個正式號碼
        if (baseData.familySerial == 0)
        {
            Debug.Log("<color=yellow>父技能尚未分配流水號，正在自動生成...</color>");
            // 呼叫下面的方法，因為是 0，它只會更新 baseData 自己
            RegenerateFamilySerial(baseData);
        }
    ...
    public static void RegenerateFamilySerial(SkillData target)
    {
        ...
        // ★ 防呆分支 A：如果是新技能 (0)，只改自己，不進行搜尋
        if (oldSerial == 0)
        {
            Undo.RecordObject(target, "Generate Skill ID");

            target.familySerial = newSerial;
            UpdateSkillID(target);

            EditorUtility.SetDirty(target);
            AssetDatabase.SaveAssets();

            Debug.Log($"<color=lime>已為新技能分配流水號: {newSerial}</color>");
            return; // 結束，不跑下面的迴圈
        }

        // ★ 分支 B：如果是舊技能 (非0)，執行家族批次更新
        string[] guids = AssetDatabase.FindAssets("t:SkillData");
        int count = 0;
